...
def model_run(
    model_name: str, options: Dict[str, Any], dic_generator: Optional[Generator] = None
) -> None:
    ...
    for opt in opt_gen:
        ...
        policy = POLICIES[model_name](env=env, **opt)
        model = SimpleModel(env, policy)
...
if __name__ == "__main__":
    ...
    for model_name, options in models.items():
        model_run(model_name, options, dic_generator=utils.grid_generator)
