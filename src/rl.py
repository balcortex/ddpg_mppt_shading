...
# from src.experience import ExperienceSource, ReplayBuffer, Experience
...
def create_mlp_actor_critic(
    ...
    actor = create_mlp_actor(env, weight_init_fn, actor_kwargs)
    critic = create_mlp_critic(env, weight_init_fn, critic_kwargs)
    return actor, critic


def create_mlp_actor(
    env: gym.Env,
    weight_init_fn: Optional[Callable] = init_weights_xavier_uniform,
    actor_kwargs: Optional[Dict[str, Union[Sequence[int], int]]] = {},
):
...
#     actor, critic = create_mlp_actor_critic(env)
